...
recordDir = "/mydata/openface/demos_hawk/makeDataset/videoFR.avi"
benchmarkGoldenDir = '/mydata/openface/demos_hawk/makeDataset/videos_chokepoint/*'
goldenDir = '/mydata/openface/demos_hawk/makeDataset/videos/*'
benchmarkVidListDir = "../benchmark_video/video_eval/*"
vidListDir = "../benchmark_video/video_eval_parlab_test/*"
groundTruthDir = "../benchmark_video/new_groundtruth/" 
...
dnn_detector = RetinaFace(retinaFaceModelDir, 0, gpuid, 'net3')
...
def find_face(img):
    faces, landmarks = dnn_detector.detect(img, fd_threshold, scales=[detect_scale], do_flip=False)
    BB = list()
...
def getRepBatch(img, facelist):
    ...
    if len(toProcess) == 0:
        return facelist
    embeddings = insightfaceModel.get_features(toProcess)
    i = 0
    for face in facelist:
        if face.freeze_prediction and freeze_on:
            continue
    ...
    for face in facelist:
        ...
        if print_landmarks:
            for loc in face.landmark:
                cv2.circle(img, (loc[0], loc[1]), 2, (255, 0, 0), -1)
...
def face_recognition_hog(frame, faces, goldenFeatures):
    ...
    if print_time:
        print("detect face = " + "{:.4f}".format(time.time() - start))
    ...
    if print_time:
        print("update_facelist = " + "{:.4f}".format(time.time() - start))
    tot_update_facelist_time += (time.time() - start)

    start = time.time()
    faces = create_facelist(frame, BB, faces)
    if print_time:
        print("create facelist = " + "{:.4f}".format(time.time() - start))
    ...
    if print_time:
        print("get representation = " + "{:.4f}".format(time.time() - start))
    tot_getRep_time += (time.time() - start)

    start = time.time()
    faces = update_prediction(faces, goldenFeatures)
    if print_time:
        print("predict face = " + "{:.4f}".format(time.time() - start))
...
def check_answer_chokepoint(frame, tree, root, facelist, frame_number, correct, num_person, false_alarm, id_correct):
    ...
    for person in root[frame_number].iter("person"):
		...
		cv2.rectangle(frame, 
			      (int(box_ans[0]), int(box_ans[1])),
                             (int(box_ans[2]), int(box_ans[3])),
			      (0, 0, 155), 4)
                break
            else:
                box_ans = [0, 0, 0, 0]
...
#initialize boostdb
    ...
    if args.record:
        ...
        video = cv2.VideoWriter(recordDir, codec_rec, 20.0, (width, height))
        ...
        for vid in vidList:
            ...
            for frame_number, frame in enumerate(frame_buffer):
                    ...
                    if args.display == True:
                        if args.benchmark:
                            frame_temp = draw_bb(frame_temp, facelist, 1/frame_time)
                        else:
                            frame_temp = draw_bb(frame.copy(), facelist, 1/frame_time)
                        try: 
                            cv2.namedWindow('image', cv2.WINDOW_NORMAL)
                            if fullscreen:
                                cv2.setWindowProperty('image', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
...
print("---statistics---")
print("avg detect face ", "{:.4f}".format(tot_detect_face_time / processed_frames))
print("avg update facelist ", "{:.4f}".format(tot_update_facelist_time / processed_frames))
print("avg create facelist", "{:.4f}".format(tot_create_facelist_time / processed_frames))
print("avg get representation", "{:.4f}".format(tot_getRep_time / processed_frames))
print("avg update prediction", "{:.4f}".format(tot_update_prediction_time / processed_frames))
